...
if [ -z "${EXERCISM_PRETTIER_VERSION:-}" ]; then
  ...
  EXERCISM_PRETTIER_VERSION="$(corepack pnpm why prettier --parseable | sed -n -e '1,$s/^.*prettier@//' -e 's/\\node_modules\\prettier//p')"
...
if [ -z "${EXERCISM_PRETTIER_VERSION:-}" ]; then
  ...
  echo ""
  echo "This is what corepack pnpm why reports:"
  echo "$ corepack pnpm why prettier --parseable"
  echo "$(corepack pnpm why prettier --parseable)"
  echo ""
...
corepack pnpm dlx "prettier@$EXERCISM_PRETTIER_VERSION" --check "**/*.{js,jsx,ts,tsx,css,sass,scss,html,json,md,yml}"
